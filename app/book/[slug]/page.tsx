...
export default async function BookingPage({ params }:{params:Promise<{slug:string}>}) {
  ...
  return <main className="shell" style={{maxWidth:760}}><nav className="nav"><span className="logo">MeetSlot</span><span className="meta">{type.timezone}</span></nav><section className="card">
    <p>Встреча с {type.user.name || "организатором"}</p><h1 style={{fontSize:48}}>{type.title}</h1><p>{type.description}</p><p><strong>{type.durationMin} минут</strong> · Google Meet</p><BookingForm slug={type.slug} timezone={type.timezone} />
  </section></main>;
}

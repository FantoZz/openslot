...
export function BookingForm({ slug, timezone }: { slug:string; timezone:string }) {
  ...
  return <form onSubmit={submit}>
    <div><h3>Выберите время</h3>{loading ? <p>Проверяем календарь…</p> : error ? null : slots.length ? <div className="slots">{slots.map(slot=><button type="button" key={slot} className={`slot ${selected===slot?"selected":""}`} onClick={()=>setSelected(slot)}>{formatter.format(new Date(slot))}</button>)}</div> : <p>Свободных окон на ближайшие 14 дней нет.</p>}</div>
